...
class Topology:
    ...
    def __init__(self, numSeqs, seqNames):
        if numSeqs < 2:
    ...
    def __findDistancePairs(self):
        ...
        for x in range(self.size):
            for y in range(x):
                tmp = distPairs.DistancePairs(self.dMatrix[x][y], x + 1, y + 1, self.names[x], self.names[y])
                dPairs.append(tmp)
    ...
    def __createNewBranch(self, sequences, tree, distancePair):
        ...
        nullNode.add_child(name="{}".format(distancePair.name2))
        nullNode.add_child(name="{}".format(distancePair.name1))
    ...
    def __addSequenceToTree(self, sequences, tree, distancePair, knownSeq):
        ...
        if (knownSeq == 1):
            parentNode = tree.search_nodes(name="nullNode{}".format(distancePair.sequence1))
        elif (knownSeq == 2):
            parentNode = tree.search_nodes(name="nullNode{}".format(distancePair.sequence2))
        
        if (len(parentNode) < 1):
            holderNode = None
        else:
            holderNode = parentNode[0]
    
        if (holderNode == None) and (knownSeq == 1):
            parentNode = tree.search_nodes(name="{}".format(distancePair.name1))
            holderNode = parentNode[0]
        elif (holderNode == None) and (knownSeq == 2):
            parentNode = tree.search_nodes(name="{}".format(distancePair.name2))
            holderNode = parentNode[0]
        else:
            holderNode.name = "null" 
        
        treeBranch = holderNode.up
        holderNode.detach()
        
        if (knownSeq == 1):
            nullNode = treeBranch.add_child(name="nullNode{}".format(distancePair.sequence1), support=distancePair.distance)
        elif (knownSeq == 2):
            nullNode = treeBranch.add_child(name="nullNode{}".format(distancePair.sequence2), support=distancePair.distance)
